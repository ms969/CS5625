...
public class Util
{
	...
	/**
	 * Utility function which draws a flat cube for displaying a cube map.
	 * 
	 * Can be used whenever you need one (e.g. postprocessing).
	 * 
	 * @param gl The OpenGL state.
	 */
	public static void drawFlatCube(GL2 gl) throws OpenGLException
	{
		/* Save which matrix is active. */
		gl.glPushAttrib(GL2.GL_TRANSFORM_BIT);
		...
		gl.glBegin(GL2.GL_QUADS);
		// POS_X
		gl.glTexCoord3f( 1.0f,  1.0f,  1.0f); gl.glVertex2f(0.0f*dx - 1.0f, 1.0f*dy - 1.0f);
		gl.glTexCoord3f( 1.0f,  1.0f, -1.0f); gl.glVertex2f(1.0f*dx - 1.0f, 1.0f*dy - 1.0f);
		gl.glTexCoord3f( 1.0f, -1.0f, -1.0f); gl.glVertex2f(1.0f*dx - 1.0f, 2.0f*dy - 1.0f);
		gl.glTexCoord3f( 1.0f, -1.0f,  1.0f); gl.glVertex2f(0.0f*dx - 1.0f, 2.0f*dy - 1.0f);
		
		// NEG_Z
		gl.glTexCoord3f( 1.0f,  1.0f, -1.0f); gl.glVertex2f(1.0f*dx - 1.0f, 1.0f*dy - 1.0f);
		gl.glTexCoord3f(-1.0f,  1.0f, -1.0f); gl.glVertex2f(2.0f*dx - 1.0f, 1.0f*dy - 1.0f);
		gl.glTexCoord3f(-1.0f, -1.0f, -1.0f); gl.glVertex2f(2.0f*dx - 1.0f, 2.0f*dy - 1.0f);
		gl.glTexCoord3f( 1.0f, -1.0f, -1.0f); gl.glVertex2f(1.0f*dx - 1.0f, 2.0f*dy - 1.0f);
		
		// NEG_X
		gl.glTexCoord3f(-1.0f,  1.0f, -1.0f); gl.glVertex2f(2.0f*dx - 1.0f, 1.0f*dy - 1.0f);
		gl.glTexCoord3f(-1.0f,  1.0f,  1.0f); gl.glVertex2f(3.0f*dx - 1.0f, 1.0f*dy - 1.0f);
		gl.glTexCoord3f(-1.0f, -1.0f,  1.0f); gl.glVertex2f(3.0f*dx - 1.0f, 2.0f*dy - 1.0f);
		gl.glTexCoord3f(-1.0f, -1.0f, -1.0f); gl.glVertex2f(2.0f*dx - 1.0f, 2.0f*dy - 1.0f);
		
		// POS_Z
		gl.glTexCoord3f(-1.0f,  1.0f,  1.0f); gl.glVertex2f(3.0f*dx - 1.0f, 1.0f*dy - 1.0f);
		gl.glTexCoord3f( 1.0f,  1.0f,  1.0f); gl.glVertex2f(4.0f*dx - 1.0f, 1.0f*dy - 1.0f);
		gl.glTexCoord3f( 1.0f, -1.0f,  1.0f); gl.glVertex2f(4.0f*dx - 1.0f, 2.0f*dy - 1.0f);
		gl.glTexCoord3f(-1.0f, -1.0f,  1.0f); gl.glVertex2f(3.0f*dx - 1.0f, 2.0f*dy - 1.0f);
		
		// POS_Y
		gl.glTexCoord3f(-1.0f,  1.0f,  1.0f); gl.glVertex2f(1.0f*dx - 1.0f, 0.0f*dy - 1.0f);
		gl.glTexCoord3f( 1.0f,  1.0f,  1.0f); gl.glVertex2f(2.0f*dx - 1.0f, 0.0f*dy - 1.0f);
		gl.glTexCoord3f( 1.0f,  1.0f, -1.0f); gl.glVertex2f(2.0f*dx - 1.0f, 1.0f*dy - 1.0f);
		gl.glTexCoord3f(-1.0f,  1.0f, -1.0f); gl.glVertex2f(1.0f*dx - 1.0f, 1.0f*dy - 1.0f);
		...
		gl.glTexCoord3f( 1.0f, -1.0f, -1.0f); gl.glVertex2f(1.0f*dx - 1.0f, 2.0f*dy - 1.0f);
		gl.glTexCoord3f(-1.0f, -1.0f, -1.0f); gl.glVertex2f(2.0f*dx - 1.0f, 2.0f*dy - 1.0f);
		gl.glTexCoord3f(-1.0f, -1.0f,  1.0f); gl.glVertex2f(2.0f*dx - 1.0f, 3.0f*dy - 1.0f);
		gl.glTexCoord3f( 1.0f, -1.0f,  1.0f); gl.glVertex2f(1.0f*dx - 1.0f, 3.0f*dy - 1.0f);
	...
	/**
	 * Clears the display and renders a fullscreen quad with the passed cube map.
	 * 
	 * @param gl The OpenGL state.
	 * @param texture The texture to display.
	 */
	public static void renderCubeMapFullscreen(GL2 gl, TextureCubeMap texture) throws OpenGLException
	{
		/* Save state and make sure the output will overwrite whatever was there. This way
		 * we don't have to waste time clearing buffers. */
		gl.glPushAttrib(GL2.GL_DEPTH_BUFFER_BIT | GL2.GL_ENABLE_BIT);
