...
void main()
{
	...
	if (HasSpecularTexture) {
		gl_FragData[2] = vec4(float(ISOTROPIC_WARD_MATERIAL_ID), texture2D(SpecularTexture, TexCoord).rgb*SpecularColor);
	} else {
		gl_FragData[2] = vec4(float(ISOTROPIC_WARD_MATERIAL_ID), SpecularColor);
	}
	
	if (HasAlphaTexture) {
		gl_FragData[3] = vec4(float(texture2D(AlphaTexture, TexCoord).x), 0.0, 0.0, 0.0);
	} else {
		gl_FragData[3] = vec4(Alpha, 0.0, 0.0, 0.0);
	}
}
