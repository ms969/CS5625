 ...
 * Renderer.java
 * 
 * The Renderer class is in charge of rendering a scene using deferred shading. This happens in 3 stages, 
 ...
 * 2. Render into gbuffer {5 = shaded scene} the final opaque scene, using all previous buffers.
 * 3. Output {5} to window.
 * 
...
public class Renderer
{
	...
	protected final int GBuffer_GradientsIndex = 4; // Unused in PA1.
	...
	public void render(GLAutoDrawable drawable, SceneObject sceneRoot, Camera camera)
	{
		...
		try
		{
			...
			/* 2. Apply deferred lighting to the g-buffer. */
			lightGBuffer(gl, camera);

			/* 3. If we're supposed to preview one gbuffer texture, do that now. 
